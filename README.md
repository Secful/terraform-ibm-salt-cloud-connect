...
1. IBM Cloud sign-in (if not already signed in) → lands on the Schematics
   Create form with repo, Terraform version, and non-secret variables
   pre-filled.
2. Paste **`salt_auth_token`** into the variables section (see note below).
3. Click **Create** → **Generate plan** → **Apply plan**.
...
| `environment_id`  | ✅ Yes (per customer) | Generated by Salt's onboarding backend          |
| `stack_id`        | Optional              | Auto-generated if empty — usually leave blank   |
| `ibm_region`      | Optional              | Defaults to `us-south`                          |
| `salt_auth_token` | ❌ **No**             | Paste into the UI form instead (see below)      |

### Why `salt_auth_token` should not be pre-filled via URL

Passing the bearer token as `tf_var_salt_auth_token=...` exposes it to
channels that a form POST avoids: the browser address bar, browser history,
the `Referer` header on any outbound asset request from that page, HTTP
access logs at intermediate proxies/CDNs, and any browser-extension or
shoulder-surfer who sees the URL.

When the customer instead pastes the token into Schematics' variable field,
it travels in a single HTTPS POST body to `cloud.ibm.com` and is stored
encrypted in the workspace's variable store. That's still "a request," but
the surface area is smaller and the token never shows up in a URL history.
...
| `environment_id`  |          | `""`       | Salt installation/environment ID, forwarded to the backend         |
